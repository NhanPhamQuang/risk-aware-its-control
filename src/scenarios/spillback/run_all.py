"""
Run all spillback risk scenarios sequentially.
Each scenario launches its own SUMO instance and exports results to CSV.

Usage:
    python -m src.scenarios.spillback.run_all                       # run all
    python -m src.scenarios.spillback.run_all junction               # by name
    python -m src.scenarios.spillback.run_all downstream cascading   # multiple
    python -m src.scenarios.spillback.run_all --list                 # show available
"""
...
SCENARIOS = {
    "junction":   ("Junction Blockage", JunctionBlockageScenario),
    "downstream": ("Downstream Capacity Reduction", DownstreamReductionScenario),
    "flood":      ("Demand Flood Spillback", DemandFloodScenario),
    "cascading":  ("Cascading Spillback", CascadingSpillbackScenario),
}
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"  SPILLBACK RISK SCENARIO RUNNER")
